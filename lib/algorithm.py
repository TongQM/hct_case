...
class Partition:
    ...
    def _CQCP_benders(self, assigned_blocks, root, prob_dict, epsilon, grid_points=20, beta=None, Lambda=None):
        ...
        for j, bi in enumerate(all_blocks):
            ...
            if bi == root:
                subgrad[bi] = g_alpha * dalpha_dzji + g_K * (K_i + F_i)
            else:
        ...
        subgrad_F_i = g_K  # ∂g/∂F_i = ci_star**-2 (same as K_i in this formulation)
        
        return best_obj, dict(zip(all_blocks, x_star)), subgrad, T_star, alpha_i, subgrad_K_i, subgrad_F_i

    def _CQCP_benders_updated(self, assigned_blocks, root, prob_dict, epsilon, K_i, F_i, grid_points=20, beta=None, Lambda=None):
        """
